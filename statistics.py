...
Statistics = collections.namedtuple("Statistics", "mean, mode, std_dev")
...
def calculate_mode(frequencies, maximum_modes):
    highest_frequency = max(frequencies.value())
    mode = [number for number, frequency in frequencies.item() if requency == highest_frequency]
